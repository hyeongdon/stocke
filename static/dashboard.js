...
function ma1592LedgerAtHtml(s, { watch = false } = {}) {
  ...
  if (!at) return '—';
  const d = parseDbUtc(at);
  const ts = esc(
    d
      ? d.toLocaleString('sv-SE', { timeZone: TZ_SEOUL })
      : String(at).slice(0, 19).replace('T', ' '),
  );
...
async function loadMa1592() {
  ...
  try {
    ...
    const tableHead =
      ...
      + `<th>종목</th><th>분석</th><th>장부상태</th><th class="num">장부 편입일시<div class="hint">최신순 · KST · 초 단위</div></th>`
      + `<th class="num">현재가</th>`
