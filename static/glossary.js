...
const GLOSSARY = [
  {
    ...
    items: [
      ...
      { t: '15/92 홀드 (ma1592)', tags: 'MA1592 1592매매 EMA15 EMA92', d: 'HTS 1592매매 편입 관찰 후 3분봉 EMA15>EMA92(GC)로 15%→35%→50% 분할 매수. 전고 반익절 없음. 시세 전 청산은 급락+DC, 시세 후는 고점트레일(기본 5%) 우선·급락+92선이탈. 하드이탈%는 수량 계산만.' },
  ...
  },
  {
    ...
    items: [
      ...
      { t: '하드이탈% (15/92)', tags: 'ma1592_hard_break_pct 사이징', d: '매수수량 계산용 가상 손절 거리. 손절가 = min(매수가×(1−손절%), EMA92×(1−하드이탈%)). 1회 리스크%로 수량을 잡음. 이 값으로 실제 매도하지 않음.', f: '수량만 · 청산 아님' },
      { t: '급락% (15/92)', tags: 'ma1592_crash_pct STOP_MA_DC_CRASH STOP_MA_CRASH', d: '최근고점 대비 하락%. 고점 이후 3봉(crash_bars) 이내여야 함. 급락만으로는 안 팔고, 시세 전은 EMA15≤EMA92(DC), 시세 후는 92선이탈과 같이 봐야 청산. 시세 후 고점트레일이 먼저 걸리면 급락을 기다리지 않음.', f: '(고점−종가)/고점 ≥ 급락%' },
      { t: '고점 트레일% (15/92)', tags: 'ma1592_trail_pct STOP_MA_TRAIL', d: '시세 후 전용. 보유고점 대비 이 % 이상 하락하면 급락·92선보다 먼저 전량 청산(STOP_MA_TRAIL). 기본 5%.', f: '(고점−현재가)/고점 ≥ 트레일%' },
      { t: '92선이탈% (15/92)', tags: 'ma1592_large_break_pct STOP_MA_CRASH', d: '시세 후 잔량 청산. 종가가 EMA92를 이 % 이상 하향 + 급락이면 STOP_MA_CRASH(급락+큰이탈). 고점트레일보다 후순위.', f: '종가 < EMA92×(1−92선이탈%)' },
